...
naming in StreamFS.  
% StreamFS uses filesystem constructs, such as hierarchical naming and symbolic
...
When a new meter is measuring a device in a room, the Energy Lens app activates the \texttt{loadcurve} process through a general
...
% In addition, it maintains a consistent view that represents the physical associations in the ERG through StreamFS.
% The relationship between plug-loads, spaces, abnd
% in the building change over time as items are moved.  
...
Placement (location) is captured with symbolic links. Node types -- specified in the metadata as an attribute value pair -- inform 
...
We distribute several ACmes~\cite{acmemeter} throughout a single floor in our building and register various plug loads.  We also tag
...
% For future work we will baseline the floor's energy consumption before and after the deployment 
...
Our most surprising observation is that the tag design is one of the most important aspects of the Energy Lens architecture. 
...
% Because QR codes are easy to produce, we provide occupants with a webpage that
...
% Since occupants are our target users, we must make the application easy to use.  For 
