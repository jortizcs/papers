...
% IMP Notes
%	mobile networks often experience low-bandwidth, high latency, and intermittent connectivy
%	Data plans are capped over mobile networks
% 	considers performance, energy usage, and wireless data consumption
%		No two are evaluated with the same metric
%		3G tail time makes it hard to predict to true quality of the connection -- it's better to send many request over 3G back to back
%		Battery lifetime must be considered
%	application typical prefetch everything, nothing, or only objects of an aribtrary size
%	
